...
func FindPaths(start, end string, stations map[string]*data.Station, numTrains int) ([][]string, [][]data.OccupationInfo, error) {
	...
	if len(allPaths) == 0 {
		return nil, nil, fmt.Errorf("%sno paths found%s", ColorRed, ColorReset)
	}
	...
	occupations := make([][]data.OccupationInfo, len(selectedPaths))
